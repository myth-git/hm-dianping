...
import com.hmdp.utils.SimpleRedisLockTest;
...
//        SECKILL_ORDER_EXECUTOR.submit(new VoucherOrderHandler());
    ...
    public Result seckillVoucherTest(Long voucherId) {
        ...
        Long userId = UserHolder.getUser().getId();
        //创建锁对象
//        SimpleRedisLockTest lockTest = new SimpleRedisLockTest("order:" + userId, stringRedisTemplate);
        RLock lockTest = redissonClient.getLock("lock:order:" + userId);
        //获取锁
//        boolean isLock = lockTest.tryLock(1200);
        boolean isLock = lockTest.tryLock();
        //判断是否获取锁成功
        if (!isLock) {
            //获取锁失败，返回错误信息或重试
        ...
        try {
            ...
            //事务生效是因为拿到这个类的代理对象，而this.createVoucherOrder并不是代理对象
            //所以要获取它的代理对象才行
//            return createVoucherOrder(voucherId);
    ...
    @Transactional
    //synchronized 加在方法上表示的是this在这个类上加锁
